...
class TestAIAnalyzer:
    ...
    def test_analyze_properties(self, mock_config_manager, sample_enriched_data, mock_boto3_client):
        ...
        with patch('renta.ai.boto3.Session') as mock_session:
            ...
            security_manager = Mock()
            security_manager.credential_manager.get_aws_session.return_value = mock_session.return_value

            analyzer = AIAnalyzer(mock_config_manager, security_manager)

            # Generate summaries
            summaries = analyzer.analyze_properties(sample_enriched_data.head(3), prompt_name="default")
...
class TestResponseParsing:
    ...
    def test_parse_claude_response(self):
        ...
        sample_response = {
            'content': [{
                'text': 'Esta propiedad presenta una excelente oportunidad...'
            }],
            'usage': {
                'input_tokens': 500,
                'output_tokens': 300
