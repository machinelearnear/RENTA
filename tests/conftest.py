...
def sample_enriched_data(sample_zonaprop_data) -> pd.DataFrame:
    ...
    enriched['match_status'] = np.random.choice(['matched', 'no_matches'], len(enriched), p=[0.8, 0.2])
    enriched['matched_listings_count'] = np.where(
        enriched['match_status'] == 'matched',
        np.random.randint(1, 10, len(enriched)),
        0
    )
    enriched['avg_airbnb_price'] = np.where(
        enriched['match_status'] == 'matched',
        np.random.uniform(50, 150, len(enriched)),
        np.nan
    )
    enriched['median_airbnb_price'] = np.where(
        enriched['match_status'] == 'matched',
        np.random.uniform(40, 140, len(enriched)),
        np.nan
    )
    enriched['avg_reviews_per_month'] = np.where(
        enriched['match_status'] == 'matched',
        np.random.uniform(0.5, 4.0, len(enriched)),
        np.nan
    )
    ...
    enriched['rental_yield_estimate'] = np.where(
        enriched['match_status'] == 'matched',
        np.random.uniform(0.03, 0.08, len(enriched)),
        np.nan
    )
