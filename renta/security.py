...
class CredentialManager:
    ...
    def validate_aws_credentials(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if error_code in ['InvalidUserID.NotFound', 'AccessDenied']:
                ...
                self.logger.error("AWS credentials invalid or insufficient permissions", error_code=error_code)
            else:
    ...
    def _scrub_account_info(self, account_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'arn' in safe_info and safe_info['arn']:
            ...
            if len(arn_parts) >= 4:
                safe_info['arn'] = f"{arn_parts[0]}:{arn_parts[1]}:{arn_parts[2]}:{arn_parts[3]}:****"
            else:
                safe_info['arn'] = "****"
...
class ConfigurationSanitizer:
    ...
    SUSPICIOUS_KEYS = {
        ...
        'apikey', 'access_key', 'secret_key', 'private_key', 'auth_token',
        'bearer_token', 'jwt', 'credential', 'credentials'
    }
    ...
    def validate_configuration(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if issues:
            ...
            self.logger.warning(
                "Configuration has security warnings",
                warning_count=len(warnings)
            )
        else:
    ...
    def _detect_hardcoded_secrets(self, config_dict: Dict[str, Any], path: str = "") -> List[Dict[str, Any]]:
    ...
    def _detect_suspicious_keys(self, config_dict: Dict[str, Any], path: str = "") -> List[Dict[str, Any]]:
        ...
        if self.pii_scrubber.enabled:
            ...
            class PIIScrubbingLogger:
                ...
                def debug(self, msg, **kwargs):
                    return self._logger.debug(self._scrubber.scrub_text(str(msg)), **self._scrub_kwargs(kwargs))
                
                def info(self, msg, **kwargs):
                    return self._logger.info(self._scrubber.scrub_text(str(msg)), **self._scrub_kwargs(kwargs))
                
                def warning(self, msg, **kwargs):
                    return self._logger.warning(self._scrubber.scrub_text(str(msg)), **self._scrub_kwargs(kwargs))
                
                def error(self, msg, **kwargs):
                    return self._logger.error(self._scrubber.scrub_text(str(msg)), **self._scrub_kwargs(kwargs))
                
                def bind(self, **kwargs):
                    return PIIScrubbingLogger(self._logger.bind(**self._scrub_kwargs(kwargs)), self._scrubber)
